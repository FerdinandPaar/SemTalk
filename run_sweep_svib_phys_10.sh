...
# ============================================================================
# SemTalk 10-run S-VIB + Physics fine-tune sweep orchestrator
#
# Behavior:
#   1) Trains 10 variants sequentially (auto-start next after previous finishes)
#   2) Every run starts from the SAME base checkpoint (same starting point)
#   3) Saves safety copies of checkpoints/config/logs into structured archive
#   4) AFTER ALL TRAINING: runs inference + subset FGD + metric extraction
#   5) Appends progress/final summary to models/DEVELOPMENT_LOG.md
#   6) Pushes metadata to GitHub periodically (configs + summaries + logs only)
#
# Usage:
#   bash run_sweep_svib_phys_10.sh
#
# Optional env overrides:
#   SWEEP_NAME=svib_phys_10runs_v1
#   RESUME_SWEEP_ROOT=outputs/sweeps/20260309_093736_svib_phys_10runs_live
#   BASE_CONFIG=configs/semtalk_moclip_sparse_ft.yaml
#   BASE_CKPT=outputs/custom/0308_220319_semtalk_moclip_sparse_ft_ft_4gpu/best_126.bin
#   START_EPOCH=116
#   END_EPOCH=156          # screening default: 40 epochs from 116
#   START_RUN_IDX=1        # 1..10 inclusive (resume from this run index)
#   END_RUN_IDX=10         # 1..10 inclusive
#   TRAIN_MODE=single        # single | ddp
#   SINGLE_GPU=0
#   DDP_NPROC=4
#   VALIDATE_DEVICE=cpu      # cpu | cuda
#   BRANCH=feature/physics-smoother-svib
#   PUSH_EVERY=1
#   PUSH_CHECKPOINTS=0       # 1 => force-add best/last bins to git (large)
#   RUN_STAT=wandb           # wandb | ts
#   WANDB_API_KEY=...        # required for non-interactive wandb on this node
#   AUTO_DETACH=1            # 1 => auto nohup self-launch if not in screen/tmux
#   EMAIL_TO=you@domain.com  # optional: mail updates after each run
#   EMAIL_FROM=semtalk@host   # optional: sender for python SMTP mode
#   SMTP_HOST=smtp.domain.com # optional SMTP; if unset, tries local `mail`
#   SMTP_PORT=587
#   SMTP_USER=...
#   SMTP_PASS=...
# ============================================================================
...
SWEEP_NAME="${SWEEP_NAME:-svib_phys_10runs_screen40_v1}"
RESUME_SWEEP_ROOT="${RESUME_SWEEP_ROOT:-}"
BASE_CONFIG="${BASE_CONFIG:-configs/semtalk_moclip_sparse_ft.yaml}"
BASE_CKPT="${BASE_CKPT:-outputs/custom/0308_220319_semtalk_moclip_sparse_ft_ft_4gpu/best_126.bin}"
START_EPOCH="${START_EPOCH:-116}"
END_EPOCH="${END_EPOCH:-156}"
...
PY
  then
    ...
    echo "        WANDB_API_KEY=... RUN_STAT=wandb bash run_sweep_svib_phys_10.sh"
    exit 1
  fi
fi
...
# 10-run matrix (single-factor sensitivity around the deployed baseline)
# Columns:
# id vib_beta_target vib_free_bits phys_lambda phys_tau_base phys_tau_floor
MATRIX=$(cat <<'EOF'
r01_base_b010_fb020_pl008_tb050_tf010 0.010 0.20 0.08 0.50 0.10
r02_b005 0.005 0.20 0.08 0.50 0.10
r03_b020 0.020 0.20 0.08 0.50 0.10
r04_fb010 0.010 0.10 0.08 0.50 0.10
r05_fb030 0.010 0.30 0.08 0.50 0.10
r06_pl005 0.010 0.20 0.05 0.50 0.10
r07_pl012 0.010 0.20 0.12 0.50 0.10
r08_tb035 0.010 0.20 0.08 0.35 0.10
r09_tb065 0.010 0.20 0.08 0.65 0.10
r10_tf005 0.010 0.20 0.08 0.50 0.05
EOF
)
...
## [$(date +%Y-%m-%d)] Auto Sweep Started — ${SWEEP_NAME}

- Start time: $(date '+%Y-%m-%d %H:%M:%S')
- Base checkpoint (shared by all 10 runs): ${BASE_CKPT}
- Epoch schedule per run: start_epoch=${START_EPOCH}, end_epoch=${END_EPOCH}
...
send_update_mail \
  "[SemTalk Sweep] Started: ${SWEEP_NAME}" \
  "Sweep started on $(hostname)\n\nName: ${SWEEP_NAME}\nBase ckpt: ${BASE_CKPT}\nEpochs: ${START_EPOCH} -> ${END_EPOCH}\nMode: ${TRAIN_MODE}\nMaster log: ${MASTER_LOG}\n"
...
EOF
  ...
  # Run training — capture exit code so set -e does NOT kill the orchestrator on failure.
  train_exit=0
  if [[ "$TRAIN_MODE" == "ddp" ]]; then
    ...
    cmd=(
      ...
      --master_port=$((29610 + run_count))
      train.py
      --config "$cfg_path"
      --ddp True
      --gpus "${gpu_arr[@]}"
      --project "semtalk_svib_phys_sweep"
      --stat "$RUN_STAT"
      --wandb_name "${SWEEP_NAME}_${run_id}"
      --notes "$notes"
    )
    MASTER_ADDR=127.0.0.1 MASTER_PORT=$((29610 + run_count)) CUDA_VISIBLE_DEVICES="$CUDA_MAP" "${cmd[@]}" > "$run_log" 2>&1 || train_exit=$?
  else
    cmd=(
      "$PYTHON" train.py
      --config "$cfg_path"
  ...
  if [[ $train_exit -ne 0 ]]; then
    echo "[WARN] ${run_id}: training process exited with code ${train_exit} (SIGKILL=137, SIGTERM=143, general crash=1) — scanning for checkpoints"
  fi

  # Locate run directory from output naming convention.
  ...
  if [[ -z "$run_dir" ]]; then
    ...
    if (( run_count % PUSH_EVERY == 0 )); then
      git_push_metadata "run ${run_id} failed (no run_dir)"
    fi
    continue
  fi

  # Safety archive per-run: copy critical files away from active output path.
  ...
  # Determine final run status: TRAINED | TRAINED_PARTIAL | FAILED_KILLED
  run_status="TRAINED"
  if [[ $train_exit -ne 0 ]]; then
    wandb_synced=$(grep -c "wandb: Synced" "$run_log" 2>/dev/null || echo 0)
    if [[ -f "${run_dir}/last_${END_EPOCH}.bin" || "${wandb_synced}" -gt 0 ]]; then
      run_status="TRAINED"
      echo "[INFO] ${run_id}: exit code ${train_exit} but epoch ${END_EPOCH} checkpoint found — marking TRAINED"
    elif [[ -n "$best_ckpt" ]]; then
      # Use the latest last_*.bin as the resume point for the next pass
      ...
      echo "[WARN] ${run_id}: training interrupted (exit ${train_exit}), last checkpoint epoch=${best_epoch} — marking TRAINED_PARTIAL (will auto-resume on next start)"
    else
      run_status="FAILED_KILLED"
      echo "[ERROR] ${run_id}: training killed (exit ${train_exit}) with no checkpoints found — marking FAILED_KILLED"
    fi
  fi
  ...
  send_update_mail \
    "[SemTalk Sweep] TRAIN done: ${run_id} (${run_status})" \
    "Run ${run_id} training finished with status ${run_status}.\nRun dir: ${run_dir}\nBest ckpt: ${best_ckpt}\n\nSweep: ${SWEEP_NAME}\n"

  if (( run_count % PUSH_EVERY == 0 )); then
    git_push_metadata "run ${run_id} training completed"
  fi
done <<< "$MATRIX"

# ---------------------------------------------------------------------------
# Post-training validation phase (only after all 10 trainings complete)
# ---------------------------------------------------------------------------
...
while IFS=, read -r run_id run_dir best_ckpt best_epoch fgd_old fid_old bc_old l1div_old status; do
  ...
  # Only validate fully completed runs
  ...
  cfg_path="${CFG_DIR}/${run_id}.yaml"
  infer_stem="demo/2_scott_0_1_1_test_${SWEEP_NAME}_${run_id}"
  val_log="${VAL_DIR}/${run_id}_validation.log"

  echo "[VAL] ${run_id}  ckpt=$(basename "$best_ckpt")"

  MASTER_ADDR=127.0.0.1 MASTER_PORT=$((29710 + ${best_epoch:-0} % 100)) \
  "$PYTHON" -m torch.distributed.run \
    --nproc_per_node=1 \
    --master_addr=127.0.0.1 \
    --master_port=$((29710 + ${best_epoch:-0} % 100)) \
    train.py \
      --config "$cfg_path" \
      --inference \
      --load_ckpt "$best_ckpt" \
  ...
  # FGD from best epoch directory generated during training validation
  fgd_line=$(
  ...
  # Rewrite CSV row (simple temp rebuild)
  ...
  send_update_mail \
    "[SemTalk Sweep] VALIDATED: ${run_id} | FGD ${fgd_val}" \
...
{
  echo "# ${SWEEP_NAME} — 10-run summary"
  echo ""
...
send_update_mail \
  "[SemTalk Sweep] COMPLETE: ${SWEEP_NAME}" \
  "Sweep completed.\nName: ${SWEEP_NAME}\nBest run: ${best_run}\nBest FGD: ${best_fgd}\n\nSummary CSV: ${SUMMARY_CSV}\nSummary MD: ${SUMMARY_MD}\n"
